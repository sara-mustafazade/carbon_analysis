...
co2_policy_long <- df_eu %>%
  ...
  select(ds_country, iso3, `2000`:`2023`) %>%
...
#since we cut data from 2004 to 2023 later policy dummy is useless, since all countries acceept policy latest in 2005
...
```{r}
# Quick data summary and save
#skimr::skim(analysis_sample)
#write_csv(analysis_sample, "analysis_sample.csv")
...
Looking at the per capita effect
```{r}
# Main spec: per-capita emissions
feols(log_co2_pc ~ lcpi + ren_c + lgdppc_c + ind_c | iso3 + year,
      cluster = ~ iso3, data = analysis_sample)

# Robustness: totals with size controls (STIRPAT-style)
feols(log_co2 ~ lcpi + log_pop + log_gdp_pc + ren_c + ind_c | iso3 + year,
      cluster = ~ iso3, data = analysis_sample)

# Intensity
feols(I(log_co2 - log_gdp) ~ lcpi + ren_c + lgdppc_c + ind_c | iso3 + year,
      cluster = ~ iso3, data = analysis_sample)
...
# Leads (-2,-1) + lags (0..3). Outcome can be log_co2_pc or log_co2.
m_leadlag <- feols(
  log_co2_pc ~ fsum(lcpi, -2:3) + ren_c + lgdppc_c + ind_c | iso3 + year,
  cluster = ~ iso3 + year, data = analysis_sample
)
iplot(m_leadlag, ref.line = 0, main = "CPI: leads(<0) and lags(>0)")

# Cumulative 3-year effect (adjust horizon as needed)
wald(m_leadlag, "fsum::lcpi_-0 + fsum::lcpi_0 + fsum::lcpi_1 + fsum::lcpi_2 + fsum::lcpi_3 = 0")
